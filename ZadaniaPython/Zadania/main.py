...
def whoLikesIt(names):
    ...
    return f"{names[0]}, {names[1]} i {likes-2} inne osoby lubią to"
...
def countSubstrings(baseString, substring):
    ...
    for i in range(len(baseString)-sub_length+1):
        if substring == baseString[i:i+sub_length]:
            counter += 1
    return counter
    raise RuntimeError()
