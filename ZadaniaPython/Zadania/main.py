...
def whoLikesIt(names):
    ...
    if len(names) == 1:
        ...
        to_print, to_count = names[:2], names[2:]
        return f"{to_print[0]}, {to_print[1]} i {len(to_count)} inne osoby lubią to"
...
def countSubstrings(baseString, substring):
    l = len(substring)
    iterations = len(baseString) // len(substring)
