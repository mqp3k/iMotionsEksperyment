...
def whoLikesIt(names):
    ...
    if size == 0:
        ...
        return f"{names[0]}, {names[2]} i {size - 2} inne osoby lubią to"
    raise RuntimeError()
...
def countSubstrings(baseString, substring):
    for index, ch in enumerate(baseString):
        if ch == substring[0]:
            for index2, ch2 in enbaseString[index:]:
